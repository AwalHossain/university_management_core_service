import { Course, CourseToFaculty, PrismaClient } from "@prisma/client";
...
const removeFaculty = async (courseId: string, payload: string[]): Promise<CourseToFaculty[] | null> => {

    await prisma.courseToFaculty.deleteMany({
        where: {
            courseId,
            facultyId: {
                in: payload
