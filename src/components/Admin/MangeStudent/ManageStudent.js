...
const ManageStudent = () => {
  ...
  useEffect(() => {
    fetch(
      `https://boiling-ocean-57836.herokuapp.com/api/getAllInfo?page=${currentPage}`
    )
      .then((res) => res.json())
  ...
  const handleDelete = (id) => {
    ...
    if (confirm) {
      fetch(
        `https://boiling-ocean-57836.herokuapp.com/api/student/info/delete/${id}`,
        {
          method: "DELETE",
          headers: {
  ...
  const handleUpdate = (id) => {
    history.push(`/admin/singleData/${id}`);
  };
  return (
    <div>
